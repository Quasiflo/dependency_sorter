...
2. You also need a Dart SDK. Most developers already have one installed, but if you prefer to go the full Mise install path, you can do so by:
   ...
   This configures mise to use the "toolchain" environment, which includes the extra dependencies from (TODO remove)
...
- `hk check --all` verifies everything (format, lints, analyzer, markdown, workflows). It doesn't modify files. `hk fix --all` on the other hand applies what it can — formatting, fixes, etc.
