...
void main() {
  group('dependency_sorter CLI', () {
    ...
    Future<ProcessResult> runCli(final List<String> args, {final String? workingDirectory}) => Process.run(
    ...
    String out(final ProcessResult result) => result.stdout as String;
    String err(final ProcessResult result) => result.stderr as String;

    void writePubspec(final String contents, [final String name = 'pubspec.yaml']) => File('${tmp.path}/$name').writeAsStringSync(contents);

    test('sorts the pubspec in place by default', () async {
      writePubspec('dependencies:\n  b: any\n  a: any\n');
      final result = await runCli([]);
      expect(result.exitCode, 0, reason: err(result));
      expect(File('${tmp.path}/pubspec.yaml').readAsStringSync(), 'dependencies:\n  a: any\n  b: any\n');
    ...
    test('resolves a directory and an explicit file path', () async {
      ...
      expect(File('${tmp.path}/pubspec.yaml').readAsStringSync(), contains('a: any\n  b: any'));
    });
